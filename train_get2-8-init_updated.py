...
def get_lr(it):
    # 1) Linear warmup for first few steps
    if it < warmup_steps:
...
for i in range(max_steps):
    ...
    if i % 10 == 0 or loss.item() < 0.1:
        if device == "mps": torch.mps.synchronize() # Wait for Mac GPU to finish for accurate timing
        t1 = time.time()
        print(f"step {i:5d} | loss: {loss.item():.6f} | lr: {lr:.2e} | norm: {norm:.4f} | dt: {(t1-t0)*1000:.2f}ms")

    if loss.item() < 0.099999:
        print(f"\nTarget loss {loss.item():.6f} reached at step {i}!")
        break
...
max_length = 150
enc = tiktoken.get_encoding('gpt2')
tokens = enc.encode("ROMEO:") # Start with a classic prompt
x = torch.tensor(tokens, dtype=torch.long).unsqueeze(0).repeat(num_return_sequences, 1)
x = x.to(device)
